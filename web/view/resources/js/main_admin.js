...
//    });
    ...
    //multiple select
    $(document).on('click', '.dropdown-menu-multiple>li>a:not(.external,.btn-open-modal)', function () {
        var checkedBoxs;
        var data = [];
...
//            sendAjax(url,'post',data,function(){
//                    openMessage(data);
//                    reloadAjaxContent();
//            });
        }
...
function initTreeCustomer() {
    var treeDiagram = $('#treeDiagram');
    var treeArr = JSON.parse($(treeDiagram).children('input[name="treeData"]').val());
    $(treeDiagram).append(JSON.stringify(treeArr));
}